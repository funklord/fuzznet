...
TEST_BINS := $(BUILD_DIR)/chain/test/chain_test \
             ...
             $(BUILD_DIR)/record/test/ledger_fuzz \
...
GUI_SRCS  += gui/config_view.cpp gui/log_view.cpp gui/sync_view.cpp \
             ...
             gui/sched_view.cpp gui/persist_view.cpp
GUI_HDRS  += gui/config_view.h gui/log_view.h gui/sync_view.h \
             ...
             gui/sched_view.h gui/persist_view.h
GUI_TSRC  += gui/test/config_view_test.cpp gui/test/log_view_test.cpp \
             ...
             gui/test/persist_view_test.cpp
endif
...
TEST_BINS += $(BUILD_DIR)/gui/test/config_view_test \
             ...
             $(BUILD_DIR)/gui/test/persist_view_test
endif
endif
...
CLI_SRCS := cli/cli.c cli/qr_print.c cli/log_print.c cli/sync_print.c \
            ...
            cli/prekey_print.c cli/persist_print.c cli/relay_print.c \
            cli/store_print.c
CLI_HDRS := cli/cli.h cli/qr_print.h cli/log_print.h cli/sync_print.h \
            ...
            cli/prekey_print.h cli/persist_print.h cli/relay_print.h \
            cli/store_print.h
CLI_TSRC := cli/test/cli_test.c cli/test/qr_print_test.c \
            ...
            cli/test/relay_print_test.c \
            cli/test/store_print_test.c
...
TEST_BINS += $(BUILD_DIR)/cli/test/cli_test \
               ...
               $(BUILD_DIR)/cli/test/relay_print_test \
...
MONO_NADM  := $(BUILD_DIR)/node/test/admin_test
...
$(BUILD_DIR)/record/test/ledger_fuzz: $(BUILD_DIR)/record/test/ledger_fuzz.o \
                                     $(BUILD_DIR)/record/ledger.o \
                                     $(BUILD_DIR)/constant_time/constant_time.o
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $^ -o $@
...
$(BUILD_DIR)/cli/test/claim_print_test: \
	$(BUILD_DIR)/cli/test/claim_print_test.o \
	$(BUILD_DIR)/cli/claim_print.o \
	$(BUILD_DIR)/claim/claim.o
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $^ -o $@

$(BUILD_DIR)/cli/test/prekey_print_test: \
	$(BUILD_DIR)/cli/test/prekey_print_test.o \
	$(BUILD_DIR)/cli/prekey_print.o \
	$(BUILD_DIR)/prekey/prekey.o \
	$(BUILD_DIR)/trust/trust.o \
	$(BUILD_DIR)/constant_time/constant_time.o
	@mkdir -p $(dir $@)
...
$(BUILD_DIR)/cli/test/relay_print_test: \
	$(BUILD_DIR)/cli/test/relay_print_test.o \
	$(BUILD_DIR)/cli/relay_print.o
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $^ -o $@

$(BUILD_DIR)/cli/test/store_print_test: \
	$(BUILD_DIR)/cli/test/store_print_test.o \
	$(BUILD_DIR)/cli/store_print.o
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $^ -o $@
...
# What this host recovered of its own stored state. sec 254.
$(BUILD_DIR)/gui/test/persist_view_test: \
	$(BUILD_DIR)/gui/test/persist_view_test.o \
	$(BUILD_DIR)/gui/persist_view.o \
	$(BUILD_DIR)/cli/persist_print.o
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $^ $(QT_LIBS) -o $@
...
$(BUILD_DIR)/chain/test/authz_fuzz: $(BUILD_DIR)/chain/test/authz_fuzz.o \
                                   $(BUILD_DIR)/chain/authz.o \
                                   $(BUILD_DIR)/chain/chain.o \
                                   $(BUILD_DIR)/chain/revocation.o \
                                   $(BUILD_DIR)/chain/manifest.o \
...
# fuzznet's own verbs answered by a node about itself, through the real local
# path: a socketpair, fzn_node_serve_local, and the client library reading a
# reply past the old 512 bound. pair_test's set plus the local path. sec 378.
$(BUILD_DIR)/node/test/admin_test.o: node/test/admin_test.c
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $(CPPFLAGS) -Inode -Iwire/generated -c $< -o $@
...
$(BUILD_DIR)/node/test/admin_test: $(BUILD_DIR)/node/test/admin_test.o \
              ...
              $(BUILD_DIR)/node/peer_persist.o $(BUILD_DIR)/persist/persist.o \
              $(BUILD_DIR)/node/provision.o $(BUILD_DIR)/node/remote.o \
              $(BUILD_DIR)/node/node.o $(BUILD_DIR)/node/local.o \
              $(BUILD_DIR)/local/peer.o $(BUILD_DIR)/local/line.o \
              $(BUILD_DIR)/local/vocabulary.o $(BUILD_DIR)/version/version.o \
              $(BUILD_DIR)/provision/provision.o $(BUILD_DIR)/chain/service.o \
              $(BUILD_DIR)/chain/authz.o $(BUILD_DIR)/frame/freshness.o \
              $(BUILD_DIR)/chunk/split.o $(BUILD_DIR)/chunk/reassembly.o \
              $(BUILD_DIR)/chain/sign_monocypher.o \
