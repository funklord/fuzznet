/* Tests for catalog/retention.c: what this host keeps. sec 320.
 *
...
static uint8_t ent_a[FZN_CATALOG_ENTITY_LEN];
static uint8_t ent_b[FZN_CATALOG_ENTITY_LEN];
static uint8_t ent_c[FZN_CATALOG_ENTITY_LEN];
...
{
	fzn_catalog_hold_t rows[4];
	fzn_catalog_holds_t holds;

	CHECK(fzn_catalog_holds_init(&holds, rows, 4) == FZN_CATALOG_OK,
	      "a table over caller-owned rows was refused");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 0,
	      "a table that has said nothing keeps something");
	CHECK(fzn_catalog_hold_count(&holds) == 0, "a fresh table holds a row");

	CHECK(fzn_catalog_retain_all(&holds, 1) == FZN_CATALOG_OK, "retain_all(1)");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 1,
	      "an entity with no word of its own did not follow the catalogue");
	CHECK(fzn_catalog_retain_all(&holds, 0) == FZN_CATALOG_OK, "retain_all(0)");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 0,
	      "the catalogue-wide bit did not go back off");

	/* A NULL TABLE KEEPS NOTHING rather than crashing or keeping. */
	CHECK(fzn_catalog_keeps(NULL, E(ent_a), 0) == 0, "a null table kept something");
	CHECK(fzn_catalog_retention_of(NULL, E(ent_a), 0) == FZN_CATALOG_RETAIN_DEFAULT,
	      "a null table said something about an entity");
	CHECK(fzn_catalog_hold_count(NULL) == 0, "a null table holds rows");
}
...
static void test_tri_state(void)
{
	...
	for (wide = 0; wide <= 1; wide++) {
		fzn_catalog_holds_init(&holds, rows, 4);
		fzn_catalog_retain_all(&holds, wide);

		CHECK(fzn_catalog_retain(&holds, E(ent_a),
		                           FZN_CATALOG_RETAIN_KEEP) == FZN_CATALOG_OK,
		      "KEEP was refused (wide=%d)", wide);
		CHECK(fzn_catalog_retain(&holds, E(ent_b),
		                           FZN_CATALOG_RETAIN_DROP) == FZN_CATALOG_OK,
		      "DROP was refused (wide=%d)", wide);

		CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 1,
		      "KEEP did not keep (wide=%d)", wide);
		CHECK(fzn_catalog_keeps(&holds, E(ent_b), 0) == 0,
		      "DROP did not drop (wide=%d)", wide);
		/* The third state: no row at all, so the bit decides. */
		CHECK(fzn_catalog_keeps(&holds, E(ent_c), 0) == wide,
		      "an entity with no row did not follow the catalogue (wide=%d)", wide);

		CHECK(fzn_catalog_retention_of(&holds, E(ent_a), 0) ==
		          FZN_CATALOG_RETAIN_KEEP, "retention_of lost KEEP (wide=%d)", wide);
		CHECK(fzn_catalog_retention_of(&holds, E(ent_b), 0) ==
		          FZN_CATALOG_RETAIN_DROP, "retention_of lost DROP (wide=%d)", wide);
		CHECK(fzn_catalog_retention_of(&holds, E(ent_c), 0) ==
		          FZN_CATALOG_RETAIN_DEFAULT,
		      "an entity with no row reported an opinion (wide=%d)", wide);
		CHECK(fzn_catalog_hold_count(&holds) == 2,
...
static void test_default_reclaims(void)
{
	...
	fzn_catalog_holds_init(&holds, rows, 2);
	fzn_catalog_retain(&holds, E(ent_a), FZN_CATALOG_RETAIN_KEEP);
	fzn_catalog_retain(&holds, E(ent_b), FZN_CATALOG_RETAIN_DROP);
	CHECK(fzn_catalog_hold_count(&holds) == 2, "two rows were not stored");

	/* Full: a third entity has nowhere to go. */
	CHECK(fzn_catalog_retain(&holds, E(ent_c), FZN_CATALOG_RETAIN_KEEP) ==
	...
	CHECK(fzn_catalog_retain(&holds, E(ent_a), FZN_CATALOG_RETAIN_DROP) ==
	          FZN_CATALOG_OK,
	      "rewriting an existing row failed because the table was full");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 0, "the rewrite did not take");
	CHECK(fzn_catalog_hold_count(&holds) == 2, "the rewrite added a row");

	CHECK(fzn_catalog_retain(&holds, E(ent_a), FZN_CATALOG_RETAIN_DEFAULT) ==
	          FZN_CATALOG_OK,
	      "DEFAULT was refused");
	CHECK(fzn_catalog_hold_count(&holds) == 1, "DEFAULT did not give the row back");
	/* And the OTHER row survived the removal -- a swap-with-last that got
	 * its indices wrong would lose or duplicate it. */
	CHECK(fzn_catalog_retention_of(&holds, E(ent_b), 0) == FZN_CATALOG_RETAIN_DROP,
	      "removing one row disturbed another");
	CHECK(fzn_catalog_retention_of(&holds, E(ent_a), 0) == FZN_CATALOG_RETAIN_DEFAULT,
...
static void test_deadline(void)
{
	...
	CHECK(fzn_catalog_retain_until(&holds, E(ent_a), FZN_CATALOG_RETAIN_KEEP,
	                                 100, FZN_CATALOG_RETAIN_DROP) == FZN_CATALOG_OK,
	      "keep-until-then-drop was refused");

	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 0) == 1, "before the deadline it dropped");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 99) == 1,
	      "one tick before the deadline it dropped");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 100) == 0,
	      "at the deadline it still kept -- the row changes its word AT the "
	      "deadline, or `due` and `retention_of` disagree at exactly T");
	CHECK(fzn_catalog_keeps(&holds, E(ent_a), 101) == 0, "past the deadline it kept");

	/* THE WEAKER FORM: keep until T, afterwards follow the catalogue. */
	fzn_catalog_retain_until(&holds, E(ent_b), FZN_CATALOG_RETAIN_KEEP, 100,
	                           FZN_CATALOG_RETAIN_DEFAULT);
	fzn_catalog_retain_all(&holds, 1);
	CHECK(fzn_catalog_keeps(&holds, E(ent_b), 100) == 1,
	      "a `then` of DEFAULT did not fall back to a keeping catalogue");
	fzn_catalog_retain_all(&holds, 0);
	CHECK(fzn_catalog_keeps(&holds, E(ent_b), 100) == 0,
	      "a `then` of DEFAULT did not fall back to a non-keeping catalogue");
	CHECK(fzn_catalog_keeps(&holds, E(ent_b), 99) == 1,
	...
	n = fzn_catalog_due(&holds, 99, out, 4, &dropped);
	CHECK(n == 0 && dropped == 0, "a deadline came due early (%zu, %zu)", n, dropped);
	n = fzn_catalog_due(&holds, 100, out, 4, &dropped);
	CHECK(n == 2 && dropped == 0, "both deadlines did not come due at T (%zu, %zu)",
	      n, dropped);
	CHECK(fzn_catalog_hold_count(&holds) == 2, "`due` reclaimed rows, and it must not");

	/* A row with no deadline never comes due. */
	fzn_catalog_retain(&holds, E(ent_c), FZN_CATALOG_RETAIN_KEEP);
	n = fzn_catalog_due(&holds, 0xffffffffffffffffu, out, 4, &dropped);
	...
	n = fzn_catalog_due(&holds, 100, out, 1, &dropped);
	CHECK(n == 1 && dropped == 1, "a too-small buffer did not report the rest (%zu, %zu)",
	      n, dropped);
	n = fzn_catalog_due(&holds, 100, out, 0, &dropped);
...
static void test_refusals(void)
{
	...
	CHECK(fzn_catalog_retain_until(&holds, E(ent_a), FZN_CATALOG_RETAIN_DEFAULT,
	                                 100, FZN_CATALOG_RETAIN_DROP) ==
	          FZN_CATALOG_ERR_MALFORMED,
	      "a deadline on a DEFAULT mode was stored");
	CHECK(fzn_catalog_retain_until(&holds, E(ent_a), FZN_CATALOG_RETAIN_DEFAULT, 0,
	                                 FZN_CATALOG_RETAIN_DROP) == FZN_CATALOG_OK,
	      "the control -- DEFAULT with no deadline -- was refused too");
	CHECK(fzn_catalog_hold_count(&holds) == 0, "a refused row was stored anyway");

	/* AN UNKNOWN MODE IS REFUSED, NOT SKIPPED. */
	CHECK(fzn_catalog_retain(&holds, E(ent_a), (fzn_catalog_retention_t)7) ==
	          FZN_CATALOG_ERR_KIND,
	      "a mode outside the three was accepted");
	CHECK(fzn_catalog_retain_until(&holds, E(ent_a), FZN_CATALOG_RETAIN_KEEP, 100,
	                                 (fzn_catalog_retention_t)7) ==
	          FZN_CATALOG_ERR_KIND,
	...
	CHECK(fzn_catalog_retain(&holds, ent_b, FZN_CATALOG_ENTITY_LEN - 1u,
	                           FZN_CATALOG_RETAIN_KEEP) == FZN_CATALOG_ERR_MALFORMED,
	      "a short entity was accepted");
	CHECK(fzn_catalog_retain(&holds, ent_b, FZN_CATALOG_ENTITY_LEN + 1u,
	                           FZN_CATALOG_RETAIN_KEEP) == FZN_CATALOG_ERR_MALFORMED,
	      "a long entity was accepted");
	CHECK(fzn_catalog_retention_of(&holds, ent_b, FZN_CATALOG_ENTITY_LEN - 1u, 0) ==
	          FZN_CATALOG_RETAIN_DEFAULT,
	      "a short entity was looked up rather than refused");
	CHECK(fzn_catalog_retain(&holds, E(ent_b), FZN_CATALOG_RETAIN_KEEP) ==
	          FZN_CATALOG_OK,
	      "the control -- a full-length entity -- was refused too");

	/* Arguments. */
	CHECK(fzn_catalog_holds_init(NULL, rows, 4) == FZN_CATALOG_ERR_MALFORMED,
	      "a null table initialised");
	CHECK(fzn_catalog_holds_init(&holds, NULL, 4) == FZN_CATALOG_ERR_MALFORMED,
	      "a capacity with no rows initialised");
	CHECK(fzn_catalog_holds_init(&holds, NULL, 0) == FZN_CATALOG_OK,
	      "a table with no capacity at all was refused, and it is the shape a "
	      "consumer whose whole policy is the wide bit wants");
	CHECK(fzn_catalog_retain(&holds, E(ent_a), FZN_CATALOG_RETAIN_KEEP) ==
	          FZN_CATALOG_ERR_RANGE,
	      "a zero-capacity table stored a row");
	CHECK(fzn_catalog_retain_all(NULL, 1) == FZN_CATALOG_ERR_MALFORMED,
	      "a null table took a wide bit");
	CHECK(fzn_catalog_retain(NULL, E(ent_a), FZN_CATALOG_RETAIN_KEEP) ==
	          FZN_CATALOG_ERR_MALFORMED,
	      "a null table took a row");
	CHECK(fzn_catalog_retain(&holds, NULL, FZN_CATALOG_ENTITY_LEN,
	                           FZN_CATALOG_RETAIN_KEEP) == FZN_CATALOG_ERR_MALFORMED,
	      "a null entity was accepted");

	fzn_catalog_holds_init(&holds, rows, 4);
	CHECK(fzn_catalog_due(&holds, 0, out, 2, NULL) == 0,
	      "`due` answered without somewhere to report the remainder");
	CHECK(fzn_catalog_due(NULL, 0, out, 2, &dropped) == 0, "`due` read a null table");
}
...
static void test_a_near_miss_is_another_entity(void)
{
	fzn_catalog_hold_t rows[4];
	fzn_catalog_holds_t holds;
	uint8_t near_a[FZN_CATALOG_ENTITY_LEN];
	uint8_t near_b[FZN_CATALOG_ENTITY_LEN];
	...
	fzn_catalog_holds_init(&holds, rows, 4);
	CHECK(fzn_catalog_retain(&holds, E(near_a), FZN_CATALOG_RETAIN_KEEP) ==
	          FZN_CATALOG_OK, "the first near-miss entity was refused");
	CHECK(fzn_catalog_retain(&holds, E(near_b), FZN_CATALOG_RETAIN_DROP) ==
	          FZN_CATALOG_OK, "the second near-miss entity was refused");
	CHECK(fzn_catalog_hold_count(&holds) == 2,
	      "two entities differing in their last byte share one row, so a "
	      "decision about one file is applied to another (count=%zu)",
	      fzn_catalog_hold_count(&holds));
	CHECK(fzn_catalog_keeps(&holds, E(near_a), 0) == 1,
	      "the first near-miss entity lost its KEEP");
	CHECK(fzn_catalog_keeps(&holds, E(near_b), 0) == 0,
...
{
	CHECK(strcmp(fzn_catalog_retention_str(FZN_CATALOG_RETAIN_DEFAULT), "default") == 0,
	      "DEFAULT is not named 'default'");
	CHECK(strcmp(fzn_catalog_retention_str(FZN_CATALOG_RETAIN_KEEP), "keep") == 0,
	      "KEEP is not named 'keep'");
	CHECK(strcmp(fzn_catalog_retention_str(FZN_CATALOG_RETAIN_DROP), "drop") == 0,
	      "DROP is not named 'drop'");
	CHECK(strcmp(fzn_catalog_retention_str((fzn_catalog_retention_t)7), "unknown") == 0,
