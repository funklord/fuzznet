/* Tests for catalog/filing.c: where this host puts the bytes. sec 323.
 *
...
static void link(fzn_catalog_assertion_t *a, const uint8_t *entity,
...
static void holder_link(fzn_catalog_assertion_t *a, const uint8_t *entity,
                        const uint8_t *name, size_t name_len, const uint8_t *path,
                        size_t path_len)
{
...
static int is_path(const fzn_catalog_filing_t *f, const uint8_t *path, size_t len)
{
...
static void test_file_under(void)
{
	...
	fzn_catalog_filing_t rows[4];
	fzn_catalog_filings_t filings;
	const fzn_catalog_filing_t *got;

	link(&set[0], e1, DIM, PHOTO, 1);
	link(&set[1], e1, DIM, MUSIC, 1);

	CHECK(fzn_catalog_filings_init(&filings, rows, 4) == FZN_CATALOG_OK,
	      "a table over caller-owned rows was refused");
	CHECK(fzn_catalog_filed_under(&filings, set, 2, e1, sizeof(e1)) == NULL,
	      "an entity nobody has filed reported a filing");

	CHECK(fzn_catalog_file_under(&filings, set, 2, e1, sizeof(e1), DIM, PHOTO) ==
	          FZN_CATALOG_OK,
	      "filing under an asserted link was refused");
	got = fzn_catalog_filed_under(&filings, set, 2, e1, sizeof(e1));
	CHECK(is_path(got, PHOTO), "the filing did not come back");
	CHECK(fzn_catalog_filing_count(&filings) == 1, "one filing was not one row");

	/* EXACTLY ONCE: filing it elsewhere REPLACES rather than adding. */
	CHECK(fzn_catalog_file_under(&filings, set, 2, e1, sizeof(e1), DIM, MUSIC) ==
	          FZN_CATALOG_OK,
	      "re-filing to another asserted link was refused");
	got = fzn_catalog_filed_under(&filings, set, 2, e1, sizeof(e1));
	CHECK(is_path(got, MUSIC), "the re-filing did not take");
	CHECK(fzn_catalog_filing_count(&filings) == 1,
	      "re-filing added a second row, so an entity is filed in two places at "
	      "once and the exactly-once invariant is gone (count=%zu)",
	      fzn_catalog_filing_count(&filings));

	CHECK(fzn_catalog_unfile(&filings, e1, sizeof(e1)) == FZN_CATALOG_OK,
	      "unfiling was refused");
	CHECK(fzn_catalog_filing_count(&filings) == 0, "unfiling did not give the row back");
	CHECK(fzn_catalog_unfile(&filings, e1, sizeof(e1)) == FZN_CATALOG_OK,
...
static void test_must_be_asserted(void)
{
	...
	fzn_catalog_filings_init(&filings, rows, 4);

	/* A path nothing asserts. */
	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, sizeof(e1), DIM, MUSIC) ==
	          FZN_CATALOG_ERR_ABSENT,
	      "an entity was filed somewhere the records do not put it");
	/* A dimension nothing asserts. */
	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, sizeof(e1),
	...
	CHECK(fzn_catalog_file_under(&filings, set, 1, e2, sizeof(e2), DIM, PHOTO) ==
	          FZN_CATALOG_ERR_ABSENT,
	      "one entity was filed on another entity's link");
	CHECK(fzn_catalog_filing_count(&filings) == 0, "a refused filing stored a row");

	/* A RETRACTED LINK IS NOT A PLACE IT BELONGS. */
	link(&set[1], e2, DIM, MUSIC, 0);
	CHECK(fzn_catalog_file_under(&filings, set, 2, e2, sizeof(e2), DIM, MUSIC) ==
	          FZN_CATALOG_ERR_ABSENT,
	...
	CHECK(fzn_catalog_file_under(&filings, set, 3, e2, sizeof(e2), DIM, MUSIC) ==
	          FZN_CATALOG_ERR_ABSENT,
...
static void test_stale_filing(void)
{
	...
	fzn_catalog_filings_init(&filings, rows, 4);
	fzn_catalog_file_under(&filings, set, 2, e1, sizeof(e1), DIM, PHOTO);
	fzn_catalog_file_under(&filings, set, 2, e2, sizeof(e2), DIM, MUSIC);
	CHECK(fzn_catalog_filing_count(&filings) == 2, "two filings were not two rows");
	...
	CHECK(fzn_catalog_filing_prune(&filings, NULL, 0) == 1,
	      "an empty set left a filing standing");
	CHECK(fzn_catalog_filing_count(&filings) == 0, "the table did not empty");
}
...
static void test_a_near_miss_is_another_entity(void)
{
	...
	fzn_catalog_filings_init(&filings, rows, 4);
	CHECK(fzn_catalog_file_under(&filings, set, 2, near_a, sizeof(near_a), DIM,
	                               PHOTO) == FZN_CATALOG_OK,
	      "the first near-miss entity was refused");
	CHECK(fzn_catalog_file_under(&filings, set, 2, near_b, sizeof(near_b), DIM,
	                               MUSIC) == FZN_CATALOG_OK,
	...
	CHECK(fzn_catalog_filing_count(&filings) == 2,
	      ...
	      fzn_catalog_filing_count(&filings));
	got = fzn_catalog_filed_under(&filings, set, 2, near_a, sizeof(near_a));
	CHECK(is_path(got, PHOTO), "the first near-miss entity got the wrong path");
	got = fzn_catalog_filed_under(&filings, set, 2, near_b, sizeof(near_b));
...
static void test_refile(void)
{
	...
	fzn_catalog_refile_t job, resumed;
	...
	fzn_catalog_filings_init(&filings, rows, 4);
	fzn_catalog_file_under(&filings, set, 4, e1, sizeof(e1), DIM, PHOTO);
	fzn_catalog_file_under(&filings, set, 4, e2, sizeof(e2), DIM, PHOTO);
	 ...
	 *    files from. */
	CHECK(fzn_catalog_refile_capture(&filings, &job, moves, 4) == FZN_CATALOG_OK,
	...
	fzn_catalog_file_under(&filings, set, 4, e1, sizeof(e1), DIM, MUSIC);
	fzn_catalog_unfile(&filings, e2, sizeof(e2));

	/* 3. both paths, from the capture and from the table as it stands. */
	CHECK(fzn_catalog_refile_at(&job, &filings, set, 4, &from, &to) ==
	          FZN_CATALOG_OK,
	 ...
	 * error. */
	CHECK(fzn_catalog_refile_advance(&job) == FZN_CATALOG_OK, "advance failed");
	memcpy(&resumed, &job, sizeof(resumed));
	resumed.moves = moves;
	CHECK(fzn_catalog_refile_progress(&resumed, &done, &total) == FZN_CATALOG_OK,
	...
	CHECK(fzn_catalog_refile_at(&resumed, &filings, set, 4, &from, &to) ==
	          FZN_CATALOG_OK,
	...
	CHECK(fzn_catalog_refile_advance(&resumed) == FZN_CATALOG_OK, "advance failed");
	CHECK(fzn_catalog_refile_at(&resumed, &filings, set, 4, &from, &to) ==
	          FZN_CATALOG_ERR_RANGE,
	      "the cursor ran past the end without saying so");
	CHECK(fzn_catalog_refile_advance(&resumed) == FZN_CATALOG_ERR_RANGE,
...
static void test_capture_must_fit(void)
{
	...
	fzn_catalog_filings_init(&filings, rows, 4);
	fzn_catalog_file_under(&filings, set, 2, e1, sizeof(e1), DIM, PHOTO);
	fzn_catalog_file_under(&filings, set, 2, e2, sizeof(e2), DIM, PHOTO);

	CHECK(fzn_catalog_refile_capture(&filings, &job, moves, 1) ==
	          FZN_CATALOG_ERR_RANGE,
	...
	fzn_catalog_filings_init(&filings, rows, 4);
	CHECK(fzn_catalog_refile_capture(&filings, &job, NULL, 0) == FZN_CATALOG_OK,
...
static void test_refusals(void)
{
	...
	fzn_catalog_filing_t rows[2];
	fzn_catalog_filings_t filings;
	fzn_catalog_refile_t job;
	fzn_catalog_move_t moves[2];
	const fzn_catalog_move_t *from;
	const fzn_catalog_filing_t *to;
	uint8_t longname[FZN_CATALOG_FILING_NAME_MAX + 1u];
	uint8_t longpath[FZN_CATALOG_FILING_PATH_MAX + 1u];

	link(&set[0], e1, DIM, PHOTO, 1);
	fzn_catalog_filings_init(&filings, rows, 2);

	memset(longname, 'n', sizeof(longname));
	memset(longpath, 'p', sizeof(longpath));

	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, sizeof(e1), longname,
	                               sizeof(longname), PHOTO) == FZN_CATALOG_ERR_RANGE,
	      "a dimension name too long for a row was accepted");
	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, sizeof(e1), DIM, longpath,
	                               sizeof(longpath)) == FZN_CATALOG_ERR_RANGE,
	      "a path too long for a row was accepted -- truncating it would name a "
	      "different place and move the file there");
	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, FZN_CATALOG_ENTITY_LEN - 1u,
	                               DIM, PHOTO) == FZN_CATALOG_ERR_MALFORMED,
	      "a short entity was accepted");
	CHECK(fzn_catalog_file_under(NULL, set, 1, e1, sizeof(e1), DIM, PHOTO) ==
	          FZN_CATALOG_ERR_MALFORMED, "a null table took a filing");
	CHECK(fzn_catalog_file_under(&filings, set, 1, e1, sizeof(e1), DIM, PHOTO) ==
	          FZN_CATALOG_OK, "the control -- every argument in range");

	/* A full table still lets an existing entity be re-filed. */
	{
		fzn_catalog_assertion_t two[2];
		fzn_catalog_filing_t one_row[1];
		fzn_catalog_filings_t small;

		link(&two[0], e1, DIM, PHOTO, 1);
		link(&two[1], e2, DIM, PHOTO, 1);
		fzn_catalog_filings_init(&small, one_row, 1);
		CHECK(fzn_catalog_file_under(&small, two, 2, e1, sizeof(e1), DIM, PHOTO) ==
		          FZN_CATALOG_OK, "the first filing was refused");
		CHECK(fzn_catalog_file_under(&small, two, 2, e2, sizeof(e2), DIM, PHOTO) ==
		          FZN_CATALOG_ERR_RANGE, "a full table took a second entity");
		link(&two[0], e1, DIM, MUSIC, 1);
		CHECK(fzn_catalog_file_under(&small, two, 2, e1, sizeof(e1), DIM, MUSIC) ==
		          FZN_CATALOG_OK,
	...
	job.moves = moves;
	CHECK(fzn_catalog_refile_at(&job, &filings, set, 1, &from, &to) ==
	          FZN_CATALOG_ERR_MALFORMED, "an uncaptured job answered at the cursor");
	CHECK(fzn_catalog_refile_advance(&job) == FZN_CATALOG_ERR_MALFORMED,
	      "an uncaptured job advanced");
	{
		size_t a = 0, b = 0;

		CHECK(fzn_catalog_refile_progress(&job, &a, &b) ==
		          FZN_CATALOG_ERR_MALFORMED, "an uncaptured job reported progress");
	}
	CHECK(fzn_catalog_filings_init(&filings, NULL, 2) == FZN_CATALOG_ERR_MALFORMED,
	      "a capacity with no rows initialised");
	CHECK(fzn_catalog_filings_init(&filings, NULL, 0) == FZN_CATALOG_OK,
