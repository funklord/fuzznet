...
static int link_asserted(const fzn_catalog_assertion_t *set, size_t count,
...
static fzn_catalog_filing_t *find(const fzn_catalog_filings_t *filings,
...
static int usable(const fzn_catalog_filings_t *filings, const uint8_t *entity,
                  size_t entity_len)
{
	if (!filings || !entity)
		return 0;
...
static int row_backed(const fzn_catalog_filing_t *row,
                      const fzn_catalog_assertion_t *set, size_t count)
{
	return link_asserted(set, count, row->entity, FZN_CATALOG_ENTITY_LEN, row->name,
                                             ...
                                             size_t path_len)
{
	 ...
	 * to move it at all. */
	if (name_len > FZN_CATALOG_FILING_NAME_MAX ||
	    path_len > FZN_CATALOG_FILING_PATH_MAX)
...
{
	const fzn_catalog_filing_t *row;
...
static void insert_sorted(fzn_catalog_move_t *moves, size_t *used,
                          const fzn_catalog_move_t *move)
{
	size_t at = *used;
                                                 ...
                                                 size_t capacity)
{
	 ...
	 * catalog/sweep.h and catalog/copy.h are zeroed first for the same
	...
	for (i = 0; i < filings->used; i++) {
		const fzn_catalog_filing_t *row = &filings->rows[i];
		fzn_catalog_move_t move;

		memset(&move, 0, sizeof(move));
		memcpy(move.entity, row->entity, FZN_CATALOG_ENTITY_LEN);
                                            ...
                                            const fzn_catalog_filing_t **to)
{
	...
	*to = fzn_catalog_filed_under(filings, set, count, move->entity,
