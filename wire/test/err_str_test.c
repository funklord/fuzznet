...
static void expect(int ok, const char *subject, const char *what)
{
	...
	if (!ok) {
		failures++;
		fprintf(stderr, "  FAIL err_str_test.c: %s: %s\n", subject, what);
	}
}
...
static const char *r_cstyle(int v) { return fzn_catalog_segment_style_str((fzn_catalog_segment_style_t)v); }
...
static const struct subject SUBJECTS[] = {
	...
	{ "fzn_catalog_segment_style_str", r_cstyle, 2 },
#ifdef FZN_CLI_ON
