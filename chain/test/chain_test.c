...
#include "../chain.h"
/* For the store's definition: `fzn_chain_verify` takes one now, and chain.h
 * declares it as an incomplete type. */
                       ...
                       size_t msg_len, const uint8_t sig[FZN_SIG_LEN])
{
	...
	if (!msg || msg_len == 0) {
		fprintf(stderr, "  FAIL chain_test.c: verifier called with an empty signed region\n");
		failures++;
		return 0;
	}
...
static int stub_sign(void *ctx, uint8_t sig[FZN_SIG_LEN], const uint8_t *msg, size_t msg_len)
{
	...
	if (!msg || msg_len == 0) {
		fprintf(stderr, "  FAIL chain_test.c: signer called with an empty region\n");
		failures++;
		return 0;
	}
...
static void cap_id_near(fzn_cap_id_t *out, uint8_t seed)
{
...
static void fixture_open(struct fixture *f, size_t i)
{
	if (fzn_hop_open(f->bytes[i], FZN_HOP_LEN, &f->hops[i]) != FZN_CHAIN_OK) {
		fprintf(stderr, "  FAIL chain_test.c: the fixture built a hop that will not open\n");
		failures++;
	}
}
...
static void fixture_init(struct fixture *f)
{
	...
	if (mint_hop(f, f->bytes[0], 0, 1, 0xc0, 1000, FZN_NO_EXPIRY, 1) != FZN_CHAIN_OK ||
		...
		fprintf(stderr, "  FAIL chain_test.c: the fixture could not mint its own chain\n");
		failures++;
	}
	fixture_open(f, 0);
	fixture_open(f, 1);
...
/* THE SHIM IS HERE AND NOT AT EVERY CALL SITE (2026-08-27).
 * `fzn_chain_verify` takes a store rather than an array and a count, because
 * `used` bounded a loop over an array whose length only `capacity` knows --
 * see chain.h. This helper keeps its own (array, count) signature so that the
 * twenty-eight callers below are untouched, and wraps them in a store here.
 * A caller that means "no revocations" passes NULL, which stays NULL. */
...
static void test_encode_open_round_trip(void)
{
...

	{
		...
		CHECK(fzn_ct_memeq(fzn_hop_capability(hop), cap.b, FZN_CAP_ID_LEN),
...
static void test_revocation_is_per_grantee(void)
{
	...
	fixture_init(&f);
	memset(&rev, 0, sizeof(rev));
	memcpy(rev.capability.b, f.cap.b, FZN_CAP_ID_LEN); /* the capability in use */
...
static void test_revocation_is_per_issuer(void)
{
	...
	fixture_init(&f);
	memset(&rev, 0, sizeof(rev));
	memcpy(rev.capability.b, f.cap.b, FZN_CAP_ID_LEN);
	memcpy(rev.grantee, fzn_hop_grantee(f.hops[1]), FZN_PUBKEY_LEN);
...
static void test_an_ancestor_revokes_forward_and_never_backward(void)
{
	...
	if (mint_hop(&f, bytes[0], 0, 1, 0xc0, 1000, FZN_NO_EXPIRY, 1) != FZN_CHAIN_OK ||
		...
		fprintf(stderr, "  FAIL chain_test.c: the fixture could not mint a three-hop chain\n");
		failures++;
		return;
	}
...
static void test_a_comparison_reads_the_whole_value(void)
{
	...
	memcpy(f.cap.b, near_cap.b, FZN_CAP_ID_LEN);
	stub_reset(&f.stub);
	CHECK(run(&f, 2000, NULL, 0) == FZN_CHAIN_ERR_CHAIN_INVALID,
	...
	memcpy(rev.issuer, f.root, FZN_PUBKEY_LEN);
	memcpy(rev.capability.b, f.cap.b, FZN_CAP_ID_LEN);
	rev.capability.b[FZN_CAP_ID_LEN - 1] ^= 0xffu;
	CHECK(run(&f, 2000, &rev, 1) == FZN_CHAIN_OK,
...
static void test_bounds(void)
{
	 ...
	 * that `NULL` with a count of 3 was refused, and there is no count to
	 * be inconsistent with any more: `fzn_chain_verify` takes a store, and
	 * a store carries its own bound. The condition it stood in for -- a
	 * count describing entries that are not there -- is now judged inside
	 * `fzn_revocation_covers`, which DENIES rather than refusing, and
	 * revocation_test.c is where that belongs. */
...
static void test_forged_capability_is_refused(void)
{
	...
	cap_id((fzn_cap_id_t *)(f.bytes[0] + FZN_HOP_OFF_CAPABILITY), 0xff);
	cap_id(&f.cap, 0xff);
...
static void test_delegate(void)
{
	...
	CHECK(fzn_chain_delegate(f.hops, 2, f.root, &f.cap, 2000, grantee, FZN_NO_EXPIRY, 0,
	                         &f.sign, NULL, bytes) == FZN_CHAIN_OK,
	...
	f.stub.identity = 2;
	CHECK(fzn_chain_delegate(f.hops, 2, f.root, &f.cap, 2000, grantee, FZN_NO_EXPIRY, 0,
	                         &f.sign, NULL, bytes) == FZN_CHAIN_ERR_NOT_DELEGABLE,
	...
	{
		fzn_revocation_t rev;
		fzn_revocation_store_t revs = { &rev, 1, 1 };
		...
		CHECK(fzn_chain_delegate(f.hops, 2, f.root, &f.cap, 2000, grantee, FZN_NO_EXPIRY,
		                         0, &f.sign, &revs, bytes) == FZN_CHAIN_ERR_REVOKED,
...
#define REFUSED(call, what) \
	...
	/* fzn_hop_encode */
	{
		...
		key(k, 1);
		REFUSED(fzn_hop_encode(NULL, k, k, &(fzn_cap_id_t){ { 0 } }, 1, 2, 0), "encoding into a null buffer");
		REFUSED(fzn_hop_encode(bytes, NULL, k, &(fzn_cap_id_t){ { 0 } }, 1, 2, 0), "encoding a null grantor");
		REFUSED(fzn_hop_encode(bytes, k, NULL, &(fzn_cap_id_t){ { 0 } }, 1, 2, 0), "encoding a null grantee");
