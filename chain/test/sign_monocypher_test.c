...
int main(void)
{
	 ...
	 * unmodified, verified under this same key a moment ago. */
	{
		...
		for (size_t i = 0; i < sizeof(FORGERY) / sizeof(FORGERY[0]); i++) {
			...
			check(fzn_chain_verify(&forged, 1, pubkey, cap, 2000, &ops, NULL, 0,
	 ...
	 * left to refuse it. */
	{
		...
		check(fzn_chain_verify(&forged, 1, pubkey, cap, 1400, &ops, NULL, 0, &out) ==
		              FZN_CHAIN_OK,
		      "the control for the expiry forgery does not verify while it is live");
		check(fzn_chain_verify(&forged, 1, pubkey, cap, 2000, &ops, NULL, 0, &out) ==
	...
	check(fzn_chain_verify(&hop, 1, other_pub, cap, 1500, &ops, NULL, 0, &out) ==
