 ...
 * and the same reasoning as catalogue/sweep.c: this host's own membership is
...
static int self_holds(const fzn_catalogue_assertion_t *set, size_t count,
 ...
 *
 * DELIBERATELY BROADER THAN `fzn_catalogue_referenced`, and the two must not
 ...
 * on. */
static int known_here(const fzn_catalogue_assertion_t *set, size_t count,
                      const uint8_t *entity, size_t entity_len)
{
	size_t i;
...
static int already_listed(const fzn_catalogue_entity_t *out, size_t written,
                          const uint8_t *entity)
{
	size_t i;

	for (i = 0; i < written; i++)
		if (memcmp(out[i].b, entity, FZN_CATALOGUE_ENTITY_LEN) == 0)
			return 1;
	return 0;
}
...
static void emit(fzn_catalogue_entity_t *out, size_t out_cap,
                 fzn_catalogue_copy_t *plan, const uint8_t *entity)
{
	...
	memcpy(out[plan->written].b, entity, FZN_CATALOGUE_ENTITY_LEN);
	plan->written++;
}

static int seen_before(const fzn_catalogue_assertion_t *set, size_t upto,
                       const uint8_t *entity, size_t entity_len)
{
	size_t i;
...
static fzn_catalogue_err_t walk(const fzn_catalogue_assertion_t *set, size_t count,
                                const fzn_catalogue_holds_t *holds, int retained_only,
                                const uint8_t *self, size_t self_len, uint64_t now,
                                int want_missing,
                                fzn_catalogue_entity_t *out, size_t out_cap,
                                             ...
                                             size_t out_cap, fzn_catalogue_copy_t *plan)
{
	...
	for (i = 0; i < want_count; i++) {
		...
		if (!self_holds(set, count, wants[i].b, FZN_CATALOGUE_ENTITY_LEN, self,
		                self_len, &held)) {
			plan->incomplete++;
			continue;
		}
