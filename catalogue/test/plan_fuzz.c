/*
 * A fuzz harness for the PLANNERS over the new model: catalogue/sweep.c,
 * catalogue/copy.c and catalogue/filing.c. sec 324.
 *
...
static uint8_t entities[MAX_ENTITIES][FZN_CATALOGUE_ENTITY_LEN];
...
static void fixtures(void)
{
	...
	for (i = 0; i < MAX_ENTITIES; i++)
		memset(entities[i], (int)(0xa0u + i), FZN_CATALOGUE_ENTITY_LEN);
}
...
static size_t build_set(fzn_catalogue_assertion_t *set)
{
	size_t n = (size_t)rnd_below(MAX_ASSERTIONS + 1u);
	size_t i;

	for (i = 0; i < n; i++) {
		fzn_catalogue_assertion_t *a = &set[i];
		...
		a->entity_len = FZN_CATALOGUE_ENTITY_LEN;
		a->name = DIM;
		a->name_len = 5;
		...
		a->capability = kind == 0 ? FZN_CATALOGUE_CAP_HOLDER : FZN_CATALOGUE_CAP_NONE;
...
static int holds_here(const fzn_catalogue_assertion_t *set, size_t n,
                      const uint8_t *entity, const uint8_t *self)
{
	...
	for (i = 0; i < n; i++) {
		if (!set[i].live || set[i].capability != FZN_CATALOGUE_CAP_HOLDER)
			continue;
		if (memcmp(set[i].entity, entity, FZN_CATALOGUE_ENTITY_LEN) != 0)
			continue;
...
static size_t other_holders(const fzn_catalogue_assertion_t *set, size_t n,
                            const uint8_t *entity, const uint8_t *self)
{
	size_t i, j, count = 0;
	...
	for (i = 0; i < n; i++) {
		...
		if (memcmp(set[i].issuer, self, 32) == 0)
			continue;
		for (j = 0; j < i; j++)
			if (set[j].live && set[j].capability == FZN_CATALOGUE_CAP_HOLDER &&
			    memcmp(set[j].entity, entity, FZN_CATALOGUE_ENTITY_LEN) == 0 &&
...
static int curated_here(const fzn_catalogue_assertion_t *set, size_t n,
                        const uint8_t *entity)
{
	...
	for (i = 0; i < n; i++)
		if (set[i].live && set[i].capability != FZN_CATALOGUE_CAP_HOLDER &&
		    memcmp(set[i].entity, entity, FZN_CATALOGUE_ENTITY_LEN) == 0)
			return 1;
	return 0;
}

static size_t distinct_entities(const fzn_catalogue_assertion_t *set, size_t n)
{
	size_t i, j, d = 0;
...
{
	fzn_catalogue_assertion_t set[MAX_ASSERTIONS];
	fzn_catalogue_hold_t hold_rows[MAX_ENTITIES];
	fzn_catalogue_holds_t holds;
	fzn_catalogue_removal_t rows[MAX_ENTITIES];
	fzn_catalogue_sweep_t job;
	fzn_catalogue_sweep_plan_t plan;
	fzn_catalogue_copy_t want_plan, hold_plan;
	fzn_catalogue_entity_t out_want[MAX_ENTITIES], out_hold[MAX_ENTITIES];
	const uint8_t *self;
	...
	fzn_catalogue_holds_init(&holds, hold_rows, MAX_ENTITIES);
	fzn_catalogue_retain_all(&holds, (int)rnd_below(2u));
	for (i = 0; i < MAX_ENTITIES; i++) {
		uint32_t mode = rnd_below(3u);

		if (mode != 0)
			fzn_catalogue_retain(&holds, entities[i],
			                     FZN_CATALOGUE_ENTITY_LEN,
			                     mode == 1 ? FZN_CATALOGUE_RETAIN_KEEP
			                               : FZN_CATALOGUE_RETAIN_DROP);
	}

	if (fzn_catalogue_sweep_capture(set, n, &holds, self, 32, min_others, 0, NULL,
	                                &job, rows, MAX_ENTITIES, &plan) != FZN_CATALOGUE_OK)
	...
	for (i = 0; i < job.used; i++) {
		...
		if (fzn_catalogue_keeps(&holds, e, FZN_CATALOGUE_ENTITY_LEN, 0))
	...
	if (fzn_catalogue_copy_want(set, n, &holds, self, 32, 0, out_want, MAX_ENTITIES,
	                            &want_plan) != FZN_CATALOGUE_OK)
		FAILED("seed %u: a well-formed want walk was refused", seed);
	if (fzn_catalogue_copy_holdings(set, n, self, 32, out_hold, MAX_ENTITIES,
	                                &hold_plan) != FZN_CATALOGUE_OK)
	...
	for (i = 0; i < want_plan.written; i++) {
		...
		if (!fzn_catalogue_keeps(&holds, out_want[i].b, FZN_CATALOGUE_ENTITY_LEN, 0))
	...
	{
		fzn_catalogue_filing_t frows[MAX_ENTITIES];
		fzn_catalogue_filings_t filings;

		fzn_catalogue_filings_init(&filings, frows, MAX_ENTITIES);
		for (i = 0; i < MAX_ENTITIES; i++) {
			const uint8_t *p = PATHS[rnd_below(2u)];
			fzn_catalogue_err_t r;

			r = fzn_catalogue_file_under(&filings, set, n, entities[i],
			                             FZN_CATALOGUE_ENTITY_LEN, DIM, 5, p, 6);
			if (r == FZN_CATALOGUE_OK)
				cov->filed++;
			else if (r != FZN_CATALOGUE_ERR_ABSENT)
			...
			if (r == FZN_CATALOGUE_OK &&
			    !fzn_catalogue_filed_under(&filings, set, n, entities[i],
			                               FZN_CATALOGUE_ENTITY_LEN))
		...
		{
			size_t was = fzn_catalogue_filing_count(&filings);

			for (i = 0; i < n; i++)
				set[i].live = 0;
			for (i = 0; i < MAX_ENTITIES; i++)
				if (fzn_catalogue_filed_under(&filings, set, n, entities[i],
				                              FZN_CATALOGUE_ENTITY_LEN))
					FAILED("seed %u: a filing answered after every "
					       "assertion was retracted", seed);
			if (was && fzn_catalogue_filing_prune(&filings, set, n) != was)
