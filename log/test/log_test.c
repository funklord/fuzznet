...
int main(void)
{
	...
	fzn_log_range(&log, alice, &first, &last);
	expect(first == 0 && last == 0, "an empty log holds no range");
	expect_err(fzn_log_get(&log, alice, 1, &got), FZN_LOG_ERR_ABSENT,
	...
	expect_err(fzn_log_get(&log, alice, 2, &got), FZN_LOG_OK, "fetching a held entry");
	expect(got != NULL && got->seq == 2, "and it is the right one");
	expect_err(fzn_log_get(&log, alice, 9, &got), FZN_LOG_ERR_ABSENT,
	...
	expect_err(fzn_log_get(&log, alice, 1, &got), FZN_LOG_ERR_GONE,
	           "asking for what retention removed");
	expect_err(fzn_log_get(&log, alice, 6, &got), FZN_LOG_ERR_ABSENT,
	...
	{
		size_t n = fzn_log_read_since(&log, alice, 2, page, 8);

		expect(n == 3, "three entries after two");
		expect(n == 3 && page[0]->seq == 3 && page[1]->seq == 4 && page[2]->seq == 5,
		       "and they must be oldest first");
	}
	expect(fzn_log_read_since(&log, alice, 5, page, 8) == 0, "nothing after the newest");
	expect(fzn_log_read_since(&log, alice, 0, page, 2) == 2, "a bounded read stops at its cap");

	/* ANOTHER ISSUER IS A SEPARATE STREAM. */
	expect(fzn_log_read_since(&log, bob, 0, page, 8) == 0, "an unheard issuer has nothing");
	fzn_log_range(&log, bob, &first, &last);
	expect(first == 0 && last == 0, "and no range");
	make(&rec, 0xb2, 1);
	expect_err(fzn_log_append(&log, &rec), FZN_LOG_OK, "bob's first entry");
	expect(fzn_log_read_since(&log, bob, 0, page, 8) == 1, "which is bob's alone");
	...
	expect_err(fzn_log_get(&log, alice, 0, &got), FZN_LOG_ERR_MALFORMED, "asking for zero");
	expect_err(fzn_log_get(&log, alice, 1, NULL), FZN_LOG_ERR_MALFORMED, "nowhere to answer");
	expect(fzn_log_read_since(&log, alice, 0, page, 0) == 0, "a zero-capacity read");
	expect(fzn_log_read_since(NULL, alice, 0, page, 8) == 0, "reading a null log");
	expect(fzn_log_dropped(NULL) == 0, "a null log dropped nothing");
	fzn_log_range(NULL, alice, &first, &last);
