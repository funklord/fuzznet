...
static void check(int ok, const char *what, uint8_t s2, uint32_t m2, uint16_t c2)
{
	...
	if (!ok) {
		failures++;
		fprintf(stderr, "  FAIL agreement_test.c: %s (sender=%02x msg=%u chunks=%u)\n", what, s2, m2, c2);
	}
}
...
static void shapes(void)
{
	...
	for (size_t i = 0; i < sizeof(counts) / sizeof(counts[0]); i++) {
		...
		if (schema < 0 || code < 0) {
			failures++;
			fprintf(stderr, "  FAIL agreement_test.c: neither side could answer for chunks=%u\n", c);
			continue;
		}
		...
		if (code && !schema) {
			failures++;
			fprintf(stderr, "  FAIL agreement_test.c: chunk/reassembly.c accepts chunks=%u which the "
			       "schema refuses\n", c);
		}
		...
		if (c > 0 && c <= FZN_REASM_MAX_CHUNKS && !(schema && code)) {
			failures++;
			fprintf(stderr, "  FAIL agreement_test.c: chunks=%u is inside both bounds and was refused "
			       "(schema=%d code=%d)\n", c, schema, code);
		}
		checks++;
		if (c > FZN_REASM_MAX_CHUNKS && !(schema && !code)) {
			failures++;
			fprintf(stderr, "  FAIL agreement_test.c: chunks=%u should be schema-legal and code-refused "
			       "(schema=%d code=%d)\n", c, schema, code);
		}
	}
	...
	if (schema_accepts_shape(4, 4) || code_accepts_shape(4, 4)) {
		failures++;
		fprintf(stderr, "  FAIL agreement_test.c: index 4 of 4 was accepted by one of the two\n");
	}
	...
	if (!schema_accepts_shape(3, 4)) {
		failures++;
		fprintf(stderr, "  FAIL agreement_test.c: the schema refuses index 3 of 4, which is a legal frame\n");
	}
	checks++;
	if (code_accepts_shape(3, 4)) {
		failures++;
		fprintf(stderr, "  FAIL agreement_test.c: the reassembler accepted a last-chunk-first arrival\n");
	}
	...
	if (!schema_accepts_shape(2, 4) || !code_accepts_shape(2, 4)) {
		failures++;
		fprintf(stderr, "  FAIL agreement_test.c: index 2 of 4 was refused by one of the two\n");
	}
}
